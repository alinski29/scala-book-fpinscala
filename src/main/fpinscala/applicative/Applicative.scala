...
trait Applicative[F[_]] extends Functor[F]:
  ...
  // primitive combinators: unit / pure & map2
  def unit[A](a: => A): F[A] // pure in cats
...
object Applicative:
  ...
  object WebFormExample:
    ...
    def validateWebForm(name: String, birthdate: String, phone: String) =
      validName(name).map3(
  ...
  object WebFormExampleWithNonEmptyList:
    /*
     - a semigroup for a type A has an associative combine operation which returns an A given two input A values
